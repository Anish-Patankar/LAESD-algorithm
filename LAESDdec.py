import binascii
import math
import random
import numpy as np
import time
import os
from backports.pbkdf2 import pbkdf2_hmac

def rightRotate(n, d):
 
    return (((n >> d)|(n << (4 - d))) & 0xF)

def decrypt(block, key):
    keyLen = len(key)
    sqrtKeyLen = int(math.sqrt(keyLen))
    if ((((sqrtKeyLen)**2) == keyLen) and (keyLen%2==0)):

        block=list(block)
        key=list(key)

        sbox = [
                
        ]
           
        sbox = np.array(sbox).reshape(16, 16).tolist()
        random.seed(int(key[0],16))
        sboxShift=int((10*random.random())%(1+10*random.random()))
        for i in range(16):
                tmp = sbox[i]
                for j in range(sboxShift+int(10*random.random())):
                    elem = tmp[0]
                    for k in range(15):
                        tmp[k] = tmp[k+1]
                    tmp[15] = elem
                sbox[i] = tmp

        sboxInv=[0]*256
        sbox=[j for i in sbox for j in i]
        for i,value in enumerate(sbox):
            sboxInv[value]=i
        sboxInv = np.array(sboxInv).reshape(16, 16).tolist()


        #Round keys
        for k in range(10):
            for i in range(keyLen-1):
                c1 = int(key[i], 16)
                c2 = int(key[i+1], 16)
                c1=c1^c2
                key[i]=str(hex(c1))[2]


        for round in range(10):

            for i in range(keyLen-1,0,-1):
                c1 = int(key[i], 16)
                c2 = int(key[i-1], 16)
                c2=c1^c2
                key[i-1]=str(hex(c2))[2]


            block = np.array(block).reshape(sqrtKeyLen, sqrtKeyLen)
            block=block.tolist()

            #Matrix mul
            colMixMat = []
            for i in range (keyLen):
                random.seed(i)
                colMixMat.append(random.sample(range(1,3),1))
            colMixMat = (np.array(colMixMat).reshape(sqrtKeyLen, sqrtKeyLen)).tolist()


            for i in range(sqrtKeyLen):
                for j in range(sqrtKeyLen):
                    tmp=int(block[i][j], 16)
                    tmp=rightRotate(tmp,colMixMat[i][j])
                    block[i][j]=(str(hex(tmp)))[2]
            
            
            #Row shift
            for i in range(sqrtKeyLen):
                tmp = block[i]
                for j in range(i):
                    elem = tmp[sqrtKeyLen-1]
                    for k in range(sqrtKeyLen-1,0,-1):
                        tmp[k] = tmp[k-1]
                    tmp[0] = elem
                block[i] = tmp

            block=[j for i in block for j in i]

            for i in range(0, len(block), 2):
                c1 = int(block[i], 16)
                c2 = int(block[i+1], 16)
                substitute = str(hex(sboxInv[c1][c2]))

                if  len(substitute)==3:
                    block[i] = '0'
                    block[i+1] = substitute[2]
                else:
                    block[i] = substitute[2]
                    block[i+1] = substitute[3]

            for i in range(keyLen):
                c1 = int(block[i], 16)
                c2 = int(key[i], 16)
                c1=c1^c2
                block[i]=str(hex(c1))[2]
        
        block=''.join(block)
        block=bytes(block,'utf-8')
        return block


#startTime=time.time()

salt = binascii.unhexlify('aaef2d3f4d77ac66e9c5a6c3d8f921d1')
passwd = "p@$Sw0rD~1".encode("utf8")
key = pbkdf2_hmac("sha256", passwd, salt, 50000, 16)
key = binascii.hexlify(key)


with open('sample.txt', 'rb') as enc_file:
    hexdata = binascii.hexlify(enc_file.read())

enc_file.close()

print("The encrypted file is: ")
print(hexdata)

decryptedFile=b''
countByte = 0
while countByte < len(hexdata):
    dataBlock = hexdata[countByte:countByte+len(key)]
    if len(dataBlock) < len(key):
        dataBlock += hexdata[0:(len(key)-len(dataBlock))]
    
    strdata = dataBlock.decode()
    
    countByte += len(key)
    decryptedFile+=decrypt(strdata, key)

print("The decrypted file is:")
print(decryptedFile)


unenc_file = open('sample.txt', 'wb')
unenc_file.write(binascii.unhexlify(decryptedFile))
unenc_file.close()

#print("The time required for decrypting the file using LAESD is: ")
#print(time.time()-startTime)